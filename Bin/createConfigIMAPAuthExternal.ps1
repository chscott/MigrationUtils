...
function PrintVariables() {
	...
	PrintToLog ${psLogFile} "   imapAdminId: ${imapAdminId}"
	PrintToLog ${psLogFile} "   imapAdminPwd: ********"
...
function ValidateTMDVariables() {
	...
	if ( !${legacyServer} -Or !${notesAdminId} -Or !${notesAdminPwd} -Or !${imapAdminId} -Or !${imapAdminPwd} ) {
...
function GenerateTMD() {
	...
	Foreach-Object { `
		...
		if  ($line.equals('$LEGACYSERVER')) { `
			...
			$line.replace('$LEGACYADMINID|$LEGACYADMINPWD',"${imapAdminId}|${imapAdminPwd}") `
		} else { `
			$line `
		} `
	} | `
