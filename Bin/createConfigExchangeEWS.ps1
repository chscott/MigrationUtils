...
$exchangeImpId		= $args[3]
$exchangeImpPwd 	= $args[4]
$tmdPrefix			= "exchange_ews"
...
function PrintVariables() {
	...
	PrintToLog ${psLogFile} "   exchangeImpId: ${exchangeImpId}"
	PrintToLog ${psLogFile} "   exchangeImpPwd: ********"
...
function ValidateTMDVariables() {
	...
	if ( !${legacyServer} -Or !${notesAdminId} -Or !${notesAdminPwd} -Or !${exchangeImpId} -Or !${exchangeImpPwd} ) {
...
function GenerateTMD() {
	...
	Foreach-Object { `
		...
		if  ($line.equals('$LEGACYSERVER')) { `
			...
			$line.replace('$LEGACYADMINID|$LEGACYADMINPWD',"${exchangeImpId}|${exchangeImpPwd}") `
		} else { `
			$line `
		} `
	} | `
